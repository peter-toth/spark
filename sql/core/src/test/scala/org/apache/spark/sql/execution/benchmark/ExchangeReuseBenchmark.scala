...
object ExchangeReuseBenchmark extends SqlBasedBenchmark {
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    ...
    val benchmark = new Benchmark(s"Exchange reuse 1", testDataSize, output = output)
    ...
    spark.sparkContext.parallelize(1 to testDataSize).toDF("key")
      .createOrReplaceTempView("fastView")

    val queryString2 =
      """
        SELECT
          (SELECT min(a.key) FROM slowView AS a JOIN slowView AS b ON b.key = a.key),
          (SELECT max(a.key) FROM slowView AS a JOIN fastView AS b ON b.key = a.key)
      """.stripMargin

    val benchmark2 = new Benchmark(s"Exchange reuse 2", testDataSize, output = output)
    Seq(true, false).foreach { originalVersion =>
