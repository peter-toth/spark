...
import org.apache.spark.sql.catalyst.optimizer.{JoinSelectionHelper, NormalizeFloatingNumbers}
...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  case class JoinSelection(conf: SQLConf)
    extends Strategy with PredicateHelper with JoinSelectionHelper {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case j: Join => selectJoin(j, conf) match {
        case BroadcastHashJoin(leftKeys, rightKeys, joinType, buildSide, condition, left, right) =>
          Seq(joins.BroadcastHashJoinExec(
            leftKeys, rightKeys, joinType, buildSide, condition, planLater(left), planLater(right)))
        case ShuffledHashJoin(leftKeys, rightKeys, joinType, buildSide, condition, left, right) =>
          Seq(joins.ShuffledHashJoinExec(
            leftKeys, rightKeys, joinType, buildSide, condition, planLater(left), planLater(right)))
        case SortMergeJoin(leftKeys, rightKeys, joinType, condition, left, right) =>
          Seq(joins.SortMergeJoinExec(
            leftKeys, rightKeys, joinType, condition, planLater(left), planLater(right)))
        case CartesianProduct(left, right, condition) =>
          Seq(joins.CartesianProductExec(planLater(left), planLater(right), condition))
        case BroadcastNestedLoopJoin(left, right, buildSide, joinType, condition) =>
