...
private[spark] class ExecutorMetricsPoller(
  ...
  implicit def toConsumer[T](op: T => Unit): Consumer[T] = {
    new Consumer[T] {
      override def accept(t: T): Unit = op.apply(t)
    }
  }

  implicit def toFunction[T, R](op: T => R): Function[T, R] = {
    new Function[T, R] {
      override def apply(t: T): R = op.apply(t)
    }
  }

  implicit def toBiFunction[T, U, R](op: (T, U) => R): BiFunction[T, U, R] = {
  ...
  def poll(): Unit = {
    ...
    def updatePeaks(metrics: AtomicLongArray): Unit = {
      (0 until metrics.length).foreach { i =>
        metrics.getAndAccumulate(i, latestMetrics(i), (l: Long, r: Long) => math.max(l, r))
      }
    }

    // for each active stage, update the peaks
    stageTCMP.forEachValue(LONG_MAX_VALUE, (v: TCMP) => updatePeaks(v.peaks))

    // for each running task, update the peaks
    taskMetricPeaks.forEachValue(LONG_MAX_VALUE, (v: AtomicLongArray) => updatePeaks(v))
  }
  ...
  def onTaskStart(taskId: Long, stageId: Int, stageAttemptId: Int): Unit = {
    ...
    val countAndPeaks = stageTCMP.computeIfAbsent((stageId, stageAttemptId),
      (_: StageKey) => TCMP(new AtomicLong(0), new AtomicLongArray(ExecutorMetricType.numMetrics)))
  ...
  def onTaskCompletion(taskId: Long, stageId: Int, stageAttemptId: Int): Unit = {
    ...
    stageTCMP.computeIfPresent((stageId, stageAttemptId),
      (k: StageKey, v: TCMP) => decrementCount(k, v))
  ...
  def getExecutorUpdates(): HashMap[StageKey, ExecutorMetrics] = {
    ...
    stageTCMP.replaceAll((k: StageKey, v: TCMP) => getUpdateAndResetPeaks(k, v))
    ...
    executorUpdates.foreach { case (k, _) =>
      stageTCMP.computeIfPresent(k, (_k: StageKey, _v: TCMP) => removeIfInactive(_k, _v))
    }
