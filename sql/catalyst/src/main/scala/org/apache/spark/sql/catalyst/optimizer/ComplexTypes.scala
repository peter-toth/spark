...
object SimplifyExtractValueOps extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = {
    val pullOutGroupingExpressions = SQLConf.get.getConf(SQLConf.PULLOUT_GROUPING_EXPRESSIONS)

    plan transform {
      case a: Aggregate if !pullOutGroupingExpressions => a
      ...
      case p => p.transformExpressionsUp {
        ...
        case GetStructField(u: UpdateFields, ordinal, _)
          ...
          u.newExprs(ordinal) match {
            ...
            // so we don't need to wrap expr in If(IsNull(struct), Literal(null, expr.dataType),
            // expr)
        ...
        case GetArrayStructFields(CreateArray(elems, useStringTypeWhenEmpty), field, ordinal, _,
        _) =>
