...
/**
 * Model trained by [[FPMax]], which holds frequent itemsets.
 * @param freqItemsets frequent itemset, which is an RDD of `FreqItemset`
 * @tparam Item item type
 */
@Since("1.3.0")
class FPMaxModel[Item: ClassTag] @Since("1.3.0") (
   ...
   *
   * The model may be loaded using `FPMaxModel.load`.
   *
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    FPMaxModel.SaveLoadV1_0.save(this, path)
  }
...
@Since("2.0.0")
object FPMaxModel extends Loader[FPMaxModel[_]] {

  @Since("2.0.0")
  override def load(sc: SparkContext, path: String): FPMaxModel[_] = {
    FPMaxModel.SaveLoadV1_0.load(sc, path)
  }
  ...
  private[fpm] object SaveLoadV1_0 {
    ...
    private val thisClassName = "org.apache.spark.mllib.fpm.FPMaxModel"

    def save(model: FPMaxModel[_], path: String): Unit = {
    ...
    def load(sc: SparkContext, path: String): FPMaxModel[_] = {
    ...
    def loadImpl[Item: ClassTag](freqItemsets: DataFrame, sample: Item): FPMaxModel[Item] = {
      ...
      new FPMaxModel(freqItemsetsRDD)
    }
  }
}
...
class FPMax private (
   ...
   * @return an [[FPMaxModel]]
   *
   */
  @Since("1.3.0")
  def run[Item: ClassTag](data: RDD[Array[Item]]): FPMaxModel[Item] = {
   ...
   */
  @Since("1.3.0")
  def run[Item, Basket <: JavaIterable[Item]](data: JavaRDD[Basket]): FPMaxModel[Item] = {
  ...
  private def genFreqItemsets[Item: ClassTag](
    ...
    var x: RDD[(Int, (Int, MFITree[Int]))] = data.map (transaction =>
        genCondTransactions(transaction, itemToRank, partitioner)
      ).aggregateByKey(new FPTree[Int], partitioner.numPartitions)(
        (tree, transaction) => tree.add(transaction, 1L),
        (tree1, tree2) => tree1.merge(tree2))
      .map { case (rank, tree) =>
        (rank / 2, (rank / 2, collect(tree, new MFITree[Int])))
      }

    (1 to log2(freqItems.length).ceil.toInt).foreach( _ =>
      x = x.reduceByKey { case ((rank1, mfiTree1), (rank2, mfiTree2)) =>
        (rank1 / 2, mfiTree1.merge(mfiTree2))
      }
    )

    x.flatMap { case (_, (_, mfiTree)) => extract(mfiTree) }
      .map { case (ranks, count) => new FreqItemset(ranks.map(i => freqItems(i)).toArray, count) }
  }

  def collect[T](fpTree: FPTree[T], mfiTree: MFITree[T]): MFITree[T] = ???

  def extract[T](mfiTree: MFITree[T]): Iterator[(List[T], Long)] = ???
  ...
  private def genCondTransactions[Item: ClassTag](
      ...
      partitioner: Partitioner): (Int, Array[Int]) = {
    val output = mutable.Map.empty[Int, Array[Int]]
    // Filter the basket by frequent items pattern and sort their ranks.
    val filtered = transaction.flatMap(itemToRank.get)
    ju.Arrays.sort(filtered)
    val n = filtered.length
    (itemToRank(filtered(n - 1)), filtered)
  }
}
