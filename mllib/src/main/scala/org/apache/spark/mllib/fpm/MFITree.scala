...
/**
 * MFI-Tree data structure used in FP-Max.
 * @tparam T item type
 */
private[fpm] class MFITree[T] extends Serializable {
  ...
  /** Adds a transaction with level. */
  def add(t: Iterable[T]): this.type = {
    var level = 0
    var curr = root
    t.foreach { item =>
  ...
  /** Merges another MFI-Tree. */
  def merge(other: MFITree[T]): this.type = {
...
//  private def project(suffix: T): MFITree[T] = {
//    val tree = new MFITree[T]
...
//  }
  ...
  def transactions: Iterator[List[T]] = getTransactions(root)

  /** Returns all transactions under this node. */
  private def getTransactions(node: Node[T]): Iterator[(List[T])] = {
    if (node.children.isEmpty) Iterator.single(List(node.item))
    else node.children.iterator.flatMap { case (item, child) =>
      getTransactions(child).map(item :: _)
    }
  }
