...
import org.apache.spark.sql.execution.{LeafExecNode, RecursiveReferenceExec, SparkPlan, UnaryExecNode}
  ...
  def apply(plan: SparkPlan): SparkPlan = {
    ...
    plan.transformUp {
      case exchange: Exchange
        if exchange.collectFirst { case _: RecursiveReferenceExec => true }.isEmpty =>
