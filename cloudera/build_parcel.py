...
class Extractor(object):
    ...
    def _fixup_special_cases(self):
        self._fixup_special_case_spark3()
        self._fixup_global()
        LOG.info("Finished package-specific and global parcel fixups")

    def _fixup_special_case_spark3(self):
...
class Component(object):
    ...
    def __init__(self, path):
        """Read in the spark3_version.properties in the cloudera folder of a component"""
        assert os.path.split(os.path.dirname(path))[1] == "cloudera", "Properties file %s is not in a cloudera directory" % path
        assert os.path.basename(path) == "spark3_version.properties", "Properties file %s is not named spark3_version.properties" % path
        self.properties = {}
...
class Metadata(object):
    ...
    * alternatives.json specifies alternatives (i.e. /usr/sbin/alternatives) for
    ...
    def _scan_components(out_dir):
        ...
        paths = glob.glob(os.path.join(out_dir, "usr/lib/*/cloudera/spark3_version.properties"))
        LOG.info("Found %d spark3_version.properties files", len(paths))
        assert paths > 0, "Did not find any spark3_version.properties files!"
        components = []
        for path in paths:
    ...
    def _write_parcel_json(self):
        parcel = {
            ...
            "replaces": "SPARK",
            "depends" : "CDH (>= 6.9), CDH (<< 8)",
            "setActiveSymlink": True,
            "scripts": {
                "defines": "spark3_env.sh",
            },
            "provides": [
                "spark3",
            ],
    ...
    def _write_env_sh(self):
        env_sh = """\
#!/bin/bash
CDH_DIRNAME=${PARCEL_DIRNAME:-"SPARK3-%s"}
export CDH_SPARK3_HOME=$PARCELS_ROOT/$CDH_DIRNAME/lib/spark3
""" % self._version.version()

        out_file = os.path.join(self._meta_dir, "spark3_env.sh")
        with open(out_file, "wt") as o:
            o.write(env_sh)
    ...
    def _write_alternatives_json(self):
        ...
        binaries = ["pyspark3", "spark3-shell", "spark3-submit"]
        conf_dirs = ["spark3"]
...
class Version(object):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--spark3-version',
                            dest="spark3_version",
                            required=True,
...
class Archiver(object):
    def __init__(self, out_dir, build_dir, version, skip_archive, distro):
        ...
        self._version = version
        self._meta_dir = None
        self._parcel_dir = os.path.join(self._out_dir, "SPARK3-" + self._version.version())
    ...
    def _archive_parcel(self):
        ...
        parcel_archive = "SPARK3-" + self._version.full_version() + ".parcel"
    ...
    def _parcel_copy(self,distributions):
        src_archive = os.path.join(self._out_dir, "SPARK3-" + self._version.full_version() + ".parcel")
        for dist in distributions[1::]:
            dest_version = "%s-%s" % (self._version.version(), osinfo.parse_name(dist).parcel_label())
            dest_archive = os.path.join(self._out_dir, "SPARK3-" + dest_version + ".parcel")
