...
object BooleanSimplification extends Rule[LogicalPlan] with PredicateHelper {
  def apply(plan: LogicalPlan): LogicalPlan = plan.transformWithPruning(
    ...
    case q: LogicalPlan => q.transformExpressionsUpWithPruning(
      _.containsAnyPattern(AND, OR, NOT), ruleId)(transformExpressions.orElse(transformNots))
  }

  val transformExpressions: PartialFunction[Expression, Expression] = {
  ...
  val transformNots: PartialFunction[Expression, Expression] = {
