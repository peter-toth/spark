...
class KeyGroupedPartitioningSuite extends DistributionAndOrderingSuiteBase {
  ...
  test("clustered distribution: output partitioning should be KeyGroupedPartitioning") {
    ...
    val partitionValues = Seq(50L, 51L, 52L).map(v => InternalRow.fromSeq(Seq(v)))
    val projectedPositions = catalystDistribution.clustering.indices

    checkQueryPlan(df, catalystDistribution,
      physical.KeyedPartitioning(catalystDistribution.clustering, partitionValues, partitionValues),
      None)
  ...
  test("non-clustered distribution: no partition") {
    ...
    checkQueryPlan(df, distribution, physical.UnknownPartitioning(0), None)
  }
  ...
  test("non-clustered distribution: single partition") {
    ...
    checkQueryPlan(df, distribution,
      physical.KeyedPartitioning(distribution.clustering, partitionValues, partitionValues), None)
  }
  ...
  private def checkQueryPlan(
    ...
    grouped.foreach( g =>
      assert(scan.outputPartitioning.asInstanceOf[KeyedPartitioning].isGrouped === g)
    )
  }
  ...
  test("SPARK-48655: group by on partition keys should not introduce additional shuffle") {
    ...
    assert(groupPartitions.size == 1,
      "should contain partition groupings when grouping by partition values")
  ...
  test("SPARK-55092: Don't group partitions when not needed 2") {
    val items_partitions = Array(identity("id"))
    createTable(items, itemsColumns, items_partitions)
