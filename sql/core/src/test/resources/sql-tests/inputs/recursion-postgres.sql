...
-- inside subqueries
-- INFO: NOT SUPPORTED in this form
--SELECT count(*) FROM (
--    WITH RECURSIVE t AS (
...
SELECT t1.*, t2.*
FROM t AS t1
JOIN t AS t2 ON t1.path[1] = t2.path[1] AND array_upper(t1.path,1) = 1
...
WITH RECURSIVE search_graph AS (
  ...
  UNION ALL
  SELECT g.*, path || STRUCT(g.f, g.t), ARRAY_CONTANS(path, STRUCT(g.f, g.t))
...
-- INFO: NOT SUPPORTED forward ref
--WITH RECURSIVE
--    x(id) AS (SELECT * FROM y UNION ALL SELECT id+1 FROM x WHERE id < 5),
--    y(id) AS (values (1))
-- SELECT * FROM x;
...
--SELECT * FROM outermost ORDER BY 1;
--
-- INFO: modified works on Spark SQL
--WITH RECURSIVE outermost(x) AS (
