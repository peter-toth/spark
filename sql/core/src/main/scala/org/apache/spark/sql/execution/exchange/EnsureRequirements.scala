          ...
          if (bestSpecOpt.isDefined && bestSpecOpt.get.isCompatibleWith(specs(idx))) {
            bestSpecOpt match {
              ...
              case Some(KeyGroupedShuffleSpec(_, _, Some(joinKeyPositions))) =>
                child match {
                  case g: GroupPartitionsExec => g.copy(joinKeyPositions = Some(joinKeyPositions))
                  case _ => GroupPartitionsExec(child, joinKeyPositions = Some(joinKeyPositions))
                }
      ...
      case p: Partitioning with Expression =>
        ...
        satisfyingKeyedPartitioning match {
          case Some(k) =>
            ...
            GroupPartitionsExec(plan, commonPartitionValues = Some(sorted.map((_, 1))))
      ...
      if (isCompatible) {
        ...
        if (conf.v2BucketingPartiallyClusteredDistributionEnabled) {
          ...
          if (!canReplicateLeft && !canReplicateRight) {
            ...
            val leftLink = (left match {
              case g: GroupPartitionsExec => g.child
              case o => o
            }).logicalLink
            val rightLink = (right match {
              case g: GroupPartitionsExec => g.child
              case o => o
            }).logicalLink
        ...
        newLeft = left match {
          case g: GroupPartitionsExec => g.copy(
            joinKeyPositions = leftSpec.joinKeyPositions,
            commonPartitionValues = Some(mergedPartValues),
            reducers = leftReducers,
            applyPartialClustering = applyPartialClustering,
            replicatePartitions = replicateLeftSide)
          case _ => GroupPartitionsExec(left,
            leftSpec.joinKeyPositions,
            Some(mergedPartValues),
            leftReducers,
            applyPartialClustering,
            replicateLeftSide
          )
        }
        newRight = right match {
          case g: GroupPartitionsExec => g.copy(
            joinKeyPositions = rightSpec.joinKeyPositions,
            commonPartitionValues = Some(mergedPartValues),
            reducers = rightReducers,
            applyPartialClustering = applyPartialClustering,
            replicatePartitions = replicateRightSide)
          case _ => GroupPartitionsExec(right,
